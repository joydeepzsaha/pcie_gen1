// ---------------------------------------------------------------------------
// tb_pcie_enum_bridge_tlp -- Stage D acceptance: the WHOLE enumeration, one
// bridge level, in front of a REAL pcie_rq_rc_top.
//
//   scan_start_i -> pcie_enum_scan (Type 1 verdict) -> pcie_enum_bus (18h)
//                -> pcie_enum_scan #2 -> pcie_enum_bar #2 -> sec_enum_done_o
//                   all through the ONE pcie_cfg_txn and the widened mux
//                -> pcie_rq_if -> tlp_layer -> TX DLLP -> [bridge model]
//                   -> (Type1->Type0 transform) -> [device model]
//
// The bench plays the Data Link Layer and answers with the BDF-routing
// BridgedCompleter over the spec-golden bridge + device pair
// (enum_tb_common, Stage D increment 2): SS7.3.3 implemented literally, UR
// arms live from reset, latencies non-zero and unequal (Trap D), completer
// IDs captured per P5.6.
//
// bar_enable_i AND bridge_enable_i are HIGH here -- this is the target that
// owns the full bridged flow. Every flow-control caveat of the bar_tlp shim
// applies verbatim (RC1, zero-means-infinite, timer-from-allocation).
// ---------------------------------------------------------------------------
`timescale 1ns/1ps
module tb_pcie_enum_bridge_tlp
  import tlp_pkg::*;
...
;
  ...
  // ---- Stage D: bridge path control and second-level status ---------------
  ...
  pcie_enum_top #(
      ...
      // Stage D: the bridge path is ON -- this is its acceptance target.
