// ---------------------------------------------------------------------------
// tb_pcie_enum_bar_tlp -- the WHOLE enumeration in front of a REAL pcie_rq_rc_top.
// Commit 2b-3, integration half. THE HEADLINE OF STAGE C.
//
//   scan_start_i -> pcie_enum_scan -> [handoff] -> pcie_enum_bar
//                -> pcie_cfg_txn -> pcie_rq_if -> tlp_layer
//                -> TX DLLP -> [completer] -> RX DLLP -> ... -> enum_done_o
//
// The standalone target owns the sizing/assignment/enable policy against an
// invented socket. This one owns what only the real stack can answer: that the
// fifteen descriptors the two sequencers build become the TLPs Base 2.1 SS2.2.7
// p.79-80 says they should, PAYLOAD DWORDS INCLUDED, and that a real NVMe-like
// endpoint enumerates end to end from flow-control init to enum_done_o under
// realistic credit rather than saturation.
//
// ! FLOW CONTROL. tlp_layer emits ZERO TLPs and reports NO error until
// link_up_i, transmit_enable_i, fc_initialized_i and one fc_update_valid_i pulse
// with non-zero credits are ALL present -- regression RC1.
//
// !! ZERO MEANS INFINITE at FC init (Base 2.1 SS2.6.1 p.138, fn 33 p.137).
// Starving a pool needs a small FINITE advertisement with no replenishment, and
// a replenishing drip must advertise a CUMULATIVE INCREASING total because
// fc_*_i is raw CREDITS_ALLOCATED off the wire (SS2.6.1.2 p.141).
//
...
// been transmitted. CPL_TIMEOUT_CYCLES is left at the SHIPPED default so the
// starvation test exercises the value Commit 2b actually runs with.
//
// bar_enable_i is HIGH here. TAG_COUNT is 8: enumeration is single outstanding
// by construction, so nothing here needs a large tag space.
// ---------------------------------------------------------------------------
`timescale 1ns/1ps
module tb_pcie_enum_bar_tlp
  import tlp_pkg::*;
...
;
  ...
  // ---- BAR-phase control and status, enums flattened for cocotb -----------
  logic        bar_enable_i;
  logic        bar_busy_o;
  logic        enum_done_o;
  logic        enum_error_o;
  ...
  // -------------------------------------------------------------------------
  // The DUT: the whole enumeration assembly -- scan, bar, the one pcie_cfg_txn
  // and the real static handoff mux.
  // -------------------------------------------------------------------------
  pcie_enum_top #(
      ...
      // The BAR phase is ON here -- this target is the one that owns it.
